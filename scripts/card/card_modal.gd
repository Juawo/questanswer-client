...
func close_modal_animation():
	...
	if(is_instance_valid(current_card_scene)):
		current_card_scene.mouse_filter = Control.MOUSE_FILTER_IGNORE
		
	var tween = create_tween()
	tween.set_parallel(true)
	tween.tween_property(current_card_scene, "scale", Vector2(0.3,0.3), 0.4).set_trans(Tween.TRANS_BACK)
